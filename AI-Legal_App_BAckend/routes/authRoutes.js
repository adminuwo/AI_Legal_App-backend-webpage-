...
router.post("/login", async (req, res) => {
  try {
    ...
    // Super Admin auto-seeding driven strictly by environment variables
    const adminEmail = (process.env.ADMIN_EMAIL || '').toLowerCase().trim();
    const adminPassword = process.env.ADMIN_PASSWORD;

    if (adminEmail && adminPassword && email && email.toLowerCase().trim() === adminEmail) {
      let adminUser = await UserModel.findOne({ email: adminEmail });
      if (!adminUser) {
        console.log(`[Auth] Seeding Super Admin user: ${adminEmail}`);
        ...
        console.log(`[Auth] Upgraded ${adminEmail} to SUPER_ADMIN role`);
      }
    }
    ...
    if (!isCorrect) {
      ...
      if (user.failedAttempts >= 5) {
        user.lockoutUntil = new Date(Date.now() + 15 * 60 * 1000); // 15 mins cooling
      }
      await user.save();
...
<body>
    ...
    <div class="card" id="login-card" style="display: none;">
        <svg class="logo-svg" viewBox="${config.viewBox || '0 0 24 24'}"><path d="${config.logo}"/></svg>
...
router.get("/google/expo-login", (req, res) => {
  ...
  res.send(`
    ...
    <html>
    <head>
      <title>Google Sign-In | AI Legal™</title>
      ...
      <style>
        ...
        h2 { color: #FFFFFF; margin: 0 0 8px 0; font-size: 20px; font-weight: 800; }
        p { color: #94A3B8; font-size: 13px; margin: 0 0 28px 0; line-height: 1.5; }
        .spinner {
        ...
        .footer-note { margin-top: 24px; font-size: 11px; color: #64748B; line-height: 1.4; }
      </style>
    </head>
    <body>
      <div class="container">
        <div class="logo-scale">⚖️</div>
        <h2>Sign in to AI Legal</h2>
        <p>Connecting securely to Google Accounts...</p>
        ...
        <div class="footer-note">If the Google selection popup doesn't open automatically, please click the button above.</div>
      </div>
      <script>
        function handleCredentialResponse(response) {
          ...
          if (redirectUri) {
            window.location.href = redirectUri + (redirectUri.includes('?') ? '&' : '?') + "credential=" + encodeURIComponent(credential);
          } else {
            alert("Error: Missing redirect URI");
          }
        }
    ...
    if (isJwt) {
      try {
        ...
        try {
          ...
          console.warn("[Google Login] Audience check fallback, decoding JWT payload:", audErr.message);
          try {
            ...
            console.error("[Google Login] Decode error:", decodeErr.message);
          }
        }
        ...
        return res.status(401).json({ error: "Invalid or expired Google ID Token", details: jwtErr.message });
      }
    } else {
...
// POST /apple — called by native app after Apple authentication succeeds
router.post("/apple", async (req, res) => {
  try {
    const { identityToken, email: bodyEmail, name: bodyName } = req.body;

    if (!identityToken) {
      return res.status(400).json({ error: "Missing required field: identityToken is required" });
    }
    ...
    if (!email) {
      return res.status(400).json({ error: "Email could not be verified from Apple account" });
    }
    ...
    // Delegate to the shared social-user handler (isRedirect=false → returns JSON)
    await handleSocialUser(profile, req, res, false);
  } catch (err) {
    console.error("[Apple Native Login Error]:", err);
    res.status(500).json({ error: "Apple login failed on server: " + err.message });
  }
});
  ...
  try {
    ...
    if (!pendingReg) {
      ...
      if (existingUser && existingUser.isVerified) {
        return res.status(400).json({ success: false, message: "User is already verified and active. Please log in." });
      }
      return res.status(404).json({ success: false, error: "Pending registration session not found. Please fill registration details again." });
    }

    // Step 9: Cooldown check (30 seconds)
    if (pendingReg.resendCooldownUntil && pendingReg.resendCooldownUntil > new Date()) {
      const remainSecs = Math.ceil((pendingReg.resendCooldownUntil.getTime() - Date.now()) / 1000);
      return res.status(429).json({ success: false, error: `Please wait ${remainSecs} seconds before requesting a new OTP.` });
    }

    // Generate new OTP & preserve previous OTP codes in history array
    const newVerificationCode = generateOTP();
    if (pendingReg.verificationCode) {
      if (!pendingReg.previousCodes) pendingReg.previousCodes = [];
      pendingReg.previousCodes.push({
        code: pendingReg.verificationCode,
        expiresAt: pendingReg.verificationCodeExpiresAt || new Date(Date.now() + 10 * 60 * 1000)
      });
    }

    pendingReg.verificationCode = newVerificationCode;
    pendingReg.verificationCodeExpiresAt = new Date(Date.now() + 10 * 60 * 1000); // 10 mins expiry
    pendingReg.resendCooldownUntil = new Date(Date.now() + 30 * 1000); // 30s cooldown
    pendingReg.attempts = 0;
    await pendingReg.save();

    await sendVerificationEmail(pendingReg.email, pendingReg.name, newVerificationCode);
