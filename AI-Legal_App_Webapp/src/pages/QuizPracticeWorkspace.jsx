    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#0F172A] text-slate-900 dark:text-white p-4 sm:p-6 lg:p-8 select-none">
      ...
      {quizState === 'idle' && (
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="p-6 rounded-3xl bg-amber-50 dark:bg-amber-950/30 border border-[#B88B2A]/50 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-2xs">
            <div className="space-y-1">
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase tracking-wider bg-[#B88B2A]/20 text-[#B88B2A] border border-[#B88B2A]/30 inline-flex items-center gap-1">
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-xl bg-[#B88B2A] hover:bg-[#b08d3b] text-white font-bold text-xs shadow-2xs transition-all flex items-center gap-2 shrink-0 cursor-pointer"
            >
          ...
          <div className="space-y-2 relative">
            <h3 className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider flex items-center gap-1.5">
              <Search className="w-3.5 h-3.5 text-[#B88B2A]" /> Global AI Legal™ Search
            </h3>
            <div className="relative">
              <input
                ...
                className="w-full pl-10 pr-4 py-3 rounded-2xl bg-white dark:bg-[#1E293B] border border-slate-200 dark:border-slate-800 text-sm font-medium text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:border-[#B88B2A] transition-colors shadow-2xs"
              />
              <Search className="w-5 h-5 text-[#B88B2A] absolute left-3.5 top-3.5" />
            </div>
              ...
              <div className="absolute top-full left-0 right-0 z-30 mt-1 bg-white dark:bg-[#1E293B] border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl overflow-hidden divide-y divide-slate-100 dark:divide-slate-800">
                {suggestionsList.map((s, idx) => (
                  <div
                    ...
                    className="p-3 hover:bg-[#B88B2A]/10 text-xs font-semibold text-slate-700 dark:text-slate-200 cursor-pointer flex items-center gap-2 transition-colors"
                  >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredTopics.map((topic) => (
              <div
                ...
                className="p-5 rounded-2xl bg-white dark:bg-[#1E293B] border border-slate-200 dark:border-slate-800 hover:border-[#B88B2A]/50 transition-all cursor-pointer shadow-2xs flex flex-col justify-between space-y-3 group"
              >
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[11px] font-bold text-[#B88B2A]">
                      {topic.questionsCount} MCQs
                    </span>
                  </div>
                  <h4 className="text-base font-bold text-[#111827] dark:text-white group-hover:text-[#B88B2A] transition-colors leading-snug">
                    {topic.title}
                  </h4>
                ...
                <div className="pt-2 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs font-bold text-[#B88B2A]">
      ...
      {quizState === 'loading' && (
        <div className="max-w-xl mx-auto py-16 text-center space-y-4">
          <div className="w-12 h-12 rounded-full border-4 border-[#B88B2A] border-t-transparent animate-spin mx-auto" />
        ...
        <div className="max-w-3xl mx-auto space-y-5">
          ...
          <div className="flex items-center justify-between text-xs font-bold">
            ...
            <button
              ...
              className="text-[#B88B2A] hover:scale-105 transition-transform cursor-pointer"
            >
              <Bookmark className={`w-5 h-5 ${bookmarkedIds.includes(questions[currentIdx]?.id) ? 'fill-[#B88B2A]' : ''}`} />
            </button>
          </div>
          ...
          <div className="w-full h-2 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
            <div
              className="h-full bg-[#B88B2A] transition-all duration-300 rounded-full"
          ...
          <div className="p-6 rounded-3xl bg-white dark:bg-[#1E293B] border border-slate-200 dark:border-slate-800 shadow-xl space-y-5">
            {questions[currentIdx]?.sectionRef && (
              <span className="px-3 py-1 rounded-full text-[11px] font-bold bg-[#B88B2A]/10 text-[#B88B2A] border border-[#B88B2A]/30 inline-block">
                {questions[currentIdx].sectionRef}
              </span>
            )}
            ...
            {!submittedAnswer ? (
              <button
                ...
                className={`w-full py-3.5 rounded-2xl font-bold text-xs transition-all cursor-pointer ${
                  selectedOption !== null
                    ? 'bg-[#B88B2A] hover:bg-[#b08d3b] text-white shadow-2xs'
              ...
              <div className="p-4 sm:p-5 rounded-2xl bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 space-y-3 text-xs leading-relaxed">
                ...
                {questions[currentIdx]?.landmarkCase && (
                  <div className="pt-2 border-t border-slate-200/80 dark:border-slate-800 text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                    <BookOpen className="w-3.5 h-3.5 text-[#B88B2A]" />
                ...
                <button
                  ...
                  className="w-full mt-2 py-3 rounded-xl bg-[#B88B2A] hover:bg-[#b08d3b] text-white font-bold text-xs transition-all shadow-2xs cursor-pointer flex items-center justify-center gap-2"
                >
      ...
      {quizState === 'completed' && (
        <div className="max-w-2xl mx-auto p-6 sm:p-8 rounded-3xl bg-white dark:bg-[#1E293B] border border-slate-200 dark:border-slate-800 shadow-2xl space-y-6 text-center">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-[#B88B2A] font-bold">{targetTopicTitle}</p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800">
            ...
            <div>
              <div className="text-lg font-black text-[#B88B2A]">{quizMode.toUpperCase()}</div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={startQuiz}
              className="flex-1 py-3 rounded-xl bg-[#B88B2A] hover:bg-[#b08d3b] text-white font-bold text-xs shadow-2xs transition-all cursor-pointer"
            >
              Retry Quiz
            </button>
          </div>
      ...
      {setupModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs select-none">
          <div className="w-full max-w-lg bg-white dark:bg-[#1E293B] rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl p-6 space-y-5">
            ...
            <p className="text-xs font-bold text-[#B88B2A]">{targetTopicTitle}</p>
                ...
                {[
                  ...
                  <div
                    ...
                    className={`p-3 rounded-xl border text-left cursor-pointer transition-all ${
                      quizMode === m.id
                        ? 'bg-[#B88B2A]/10 border-[#B88B2A] text-[#B88B2A]'
            ...
            <button
              onClick={startQuiz}
              className="w-full py-3.5 rounded-2xl bg-[#B88B2A] hover:bg-[#b08d3b] text-white font-bold text-xs shadow-2xs transition-all cursor-pointer"
            >
              🚀 Start Quiz
            </button>
          </div>
